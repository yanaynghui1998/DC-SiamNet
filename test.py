...
from skimage.metrics import peak_signal_noise_ratio, structural_similarity,normalized_root_mse,mean_squared_error
# PyTorch
...
parser.add_argument('--num-layers', type=int, default=6, help='number of iterations')
...
parser.add_argument('--u-mask-path', type=str, default='The path of Raw undersampling mask', help='undersampling mask')
parser.add_argument('--s-mask-up-path', type=str, default='The path of Sub_undersampling mask for upper branch', help='selection mask in up network')
parser.add_argument('--s-mask-down-path', type=str, default='The path of Sub_undersampling mask for lower branch', help='selection mask in down network')
parser.add_argument('--test-sample-rate', '-tesr', type=float, default=0.02, help='sampling rate of test data')#D:\SSL-MRI-reconstruction\checkpoints\random_sampling\8x
# others
parser.add_argument('--model-save-path', type=str, default='./checkpoints/random_sampling/4x', help='save path of trained model')#random_sampling/8x/
imgfile="E:/T2-image"
def validate(args):
    ...
    model_path = os.path.join(args.model_save_path, '******')# loading your trained model
    ...
    with torch.no_grad():
        ...
        for iter_num, data_batch in enumerate(t):
            ...
            output=model(under_img.permute(0, 3, 1, 2).contiguous(),mask_under)
            ...
            under_img_np, output_np, label_np = under_img.detach().cpu().numpy(), output.detach().cpu().numpy(), label.float().detach().cpu().numpy()
...
# Tensorboard_for_Visualization
    writer = SummaryWriter('reconstruction img')
    writer.add_images('rec_img',output)
    writer.close()
